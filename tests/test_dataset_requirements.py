...
def _config_for_dataset(dataset_id: str) -> WHARConfig:
    for registered_id, cfg in har_dataset_dict.items():
        if registered_id.value == dataset_id:
            return cfg
...
def _require_cached_common_format(
    ...
    activity_df_path = metadata_dir / "activity_df.parquet"
    ...
    if (
        ...
        pytest.skip(
            f"Missing cached common format files for '{dataset_id}' under "
            f"'{dataset_dir}'."
        )

    session_df = pd.read_parquet(session_df_path, engine="pyarrow")
    activity_df = pd.read_parquet(activity_df_path, engine="pyarrow")
...
def _assert_windowing_integrity(
    ...
    window_df = pd.read_parquet(window_df_path, engine="pyarrow")
