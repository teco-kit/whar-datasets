...
def session_to_wide(
    ...
    for dev in devices:
        dev_tmp = seg[seg["device"] == dev].sort_values("timestamp")
        if dev_tmp.empty:
        ...
        #für timestamp duplikate    
        dev_tmp = dev_tmp.groupby("timestamp", as_index=False).agg(
            {"x": "mean", "y": "mean", "z": "mean"}
        )

        if len(dev_tmp) < min_points:
            if require_all_devices:
                return pd.DataFrame()
            for col in ["x", "y", "z"]:
                wide[f"{dev}_{col}"] = np.nan
        ...
        t_src = (dev_tmp["timestamp"].view("int64") / 1e9).to_numpy()

        for col in ["x", "y", "z"]:
            y = dev_tmp[col].astype(float).to_numpy()
