...
def parse_wisdm_19_phone(
    ...
    for subject in range(1600, 1651):
        ...
        accel_df = accel_df.groupby(
            ...
            accel_phone_x=("accel_phone_x", "mean"),
            accel_phone_y=("accel_phone_y", "mean"),
            accel_phone_z=("accel_phone_z", "mean"),
            source_order=("source_order", "min"),
        )
        gyro_df = gyro_df.groupby(
            ["subject_id", "activity_id", "timestamp"], as_index=False, sort=False
        ).agg(
            gyro_phone_x=("gyro_phone_x", "mean"),
            gyro_phone_y=("gyro_phone_y", "mean"),
            gyro_phone_z=("gyro_phone_z", "mean"),
        )
        ...
        merged = merged.dropna(
            subset=["gyro_phone_x", "gyro_phone_y", "gyro_phone_z"]
    ...
    if unknown_activity_labels:
        raise ValueError(
            "WISDM 2019 phone contains unsupported activity labels: "
