...
def parse_wisdm_19_watch(
    ...
    base_path = os.path.join(dir, "wisdm-dataset/wisdm-dataset/raw/watch")

    for subject in range(1600, 1651):
        accel_path = os.path.join(base_path, "accel", f"data_{subject}_accel_watch.txt")
        gyro_path = os.path.join(base_path, "gyro", f"data_{subject}_gyro_watch.txt")
        ...
        accel_df["accel_watch_z"] = (
            accel_df["accel_watch_z"].astype(str).str.replace(";", "", regex=False)
        )
        gyro_df["gyro_watch_z"] = (
            gyro_df["gyro_watch_z"].astype(str).str.replace(";", "", regex=False)
        )

        for col in ["timestamp", "accel_watch_x", "accel_watch_y", "accel_watch_z"]:
